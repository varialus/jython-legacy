...
###FJW commenting out version check for now.
###version = Database.version_info
###if (version < (1,2,1) or (version[:3] == (1, 2, 1) and
###        (len(version) < 5 or version[3] != 'final' or version[4] < 2))):
###    raise ImportError, "MySQLdb-1.2.1p2 or newer is required; you have %s" % Database.__version__

###FJW commenting these out for now.
...
### FJW: commented out conversions.copy and update for now.
...
class DatabaseWrapper(local):
    ...
    def cursor(self):
        ...
        if not self._valid_connection():
            ...
            self.connection = Database.connect('jdbc:mysql://%(host)s:%(port)s/%(db)s' % kwargs, kwargs['user'], kwargs['passwd'], 'org.gjt.mm.mysql.Driver')
