...
public class CodeCompiler extends Visitor implements Opcodes, ClassConstants //, PythonGrammarTreeConstants
{
    ...
    /**
     * visitWith produces bytecodes that correspond to the following python code:
     * <code>
     *    mgr = (EXPR)
    ...
    public Object visitWith(With node) throws Exception {
        WithHandler wh = new WithHandler(node);
        excepthandlerType eh = new excepthandlerType(node, null, null, new stmtType[0], node.getLine(), node.getCharPositionInLine());
        TryExcept te = new TryExcept(node, node.body, new excepthandlerType[]{eh},
                new stmtType[0]);
        TryFinally tf = new TryFinally(node, new stmtType[]{te}, new stmtType[0]);
        doTryFinally(tf, wh);
        wh.free();
        return null;
    }
    ...
    {
        return doTryFinally(node, null);
    }

    public Object doTryFinally(TryFinally node, WithHandler withHandler) throws Exception {
        Label start = new Label();
        Label end = new Label();
        ...
        ExceptionHandler inFinally = new ExceptionHandler(node, withHandler);
        ...
        if (withHandler == null) {
            ...
            ret = doTryExcept((TryExcept)node.body[0], withHandler);
        }
    ...
    private void inlineFinally(ExceptionHandler handler) throws Exception {
        ...
        if (handler.isInWith()) {
            Label endWith = new Label();
            code.iload(handler.withHandler.getExc());
            code.ifne(endWith);
            //XXX start failure
            code.aload(handler.withHandler.getExit());
            code.invokevirtual("org/python/core/PyObject", "__call__", "(" + $pyObj + ")" + $pyObj);
            //XXX end failure
            code.aload(handler.withHandler.getExit());
            code.label(endWith);

        } else if (handler.isFinallyHandler()) {
    ...
    public Object visitTryExcept(TryExcept node) throws Exception {
        return doTryExcept(node, null);
    }

    public Object doTryExcept(TryExcept node, WithHandler withHandler) throws Exception {
        Label start = new Label();
        Label end = new Label();
        ...
        exceptionHandlers.push(handler);
        if (withHandler != null) {
            With w = withHandler.getNode();
            if (w.optional_vars != null) {
                code.aload(withHandler.getValue());
                set(w.optional_vars);
            }
        }
        Object exit = suite(node.body);
        ...
        if (withHandler != null) {
            ...
            code.iconst_0();
            code.istore(withHandler.getExc());
            //FIXME:
            //if not exit(*sys.exc_info()):
            //    raise
            code.aload(withHandler.getExit());
            code.invokestatic("org/python/core/PySystemState", "exc_info", "()Lorg/python/core/PyTuple;");
            code.invokevirtual("org/python/core/PyObject", "__call__", "(" + $pyObj + ")" + $pyObj);
            code.invokevirtual("org/python/core/PyObject", "__nonzero__", "()Z");
            code.ifne(end_of_self);
            code.aload(exc);
            code.athrow();
            code.label(end_of_self);
    ...
    class WithHandler {
        ...
        public WithHandler(With w) throws Exception {
            this.node = w;
            visit(w.context_expr);
            mgr = storeTop();
            ...
            code.iconst_1();
            exc = code.getLocal("java/lang/Boolean");
            code.istore(exc);
        }
    ...
    class ExceptionHandler {
        ...
        public WithHandler withHandler = null;

        public ExceptionHandler() {
        }

        public ExceptionHandler(TryFinally n, WithHandler wh) {
            node = n;
            withHandler = wh;
        }
