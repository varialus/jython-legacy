...
public class AstToBytecode implements VisitorIF<BytecodeBundle> {
    ...
    public BytecodeBundle visitModule(Module node) throws Exception {
        ...
        try {
            compiler = bundle.compile(signature(),
    ...
    public BytecodeBundle visitClassDef(ClassDef node) throws Exception {
        ...
        try {
            ...
            // Make a new class, in a new environment
            EnvironmentInfo oldEnvironment = currentEnvironment;
            currentEnvironment = context.getEnvironment(node);
    ...
    public BytecodeBundle visitFunctionDef(FunctionDef node) throws Exception {
        ...
        try {
            ...
            // Make a new function, in a new environment
            EnvironmentInfo oldEnvironment = currentEnvironment;
            currentEnvironment = context.getEnvironment(node);
            try {
                BytecodeCompiler oldCompiler = compiler;
                compiler = compiler.constructFunction(currentEnvironment.closureVariables(),
                                                      numDefaults,
                                                      currentEnvironment.getCompilerFlags(),
                                                      node.name);
                try {
                    ...
                    // Add a return None if the code wasn't explicit about returning
    ...
    public BytecodeBundle visitLambda(Lambda node) throws Exception {
        ...
        try {
            ...
            // Make a new lambda function in a new environment
            EnvironmentInfo oldEnvironment = currentEnvironment;
            currentEnvironment = context.getEnvironment(node);
            try {
                BytecodeCompiler oldCompiler = compiler;
    ...
    public BytecodeBundle visitGeneratorExp(GeneratorExp node) throws Exception {
        ...
        expr_action = popResult;
        try {
            // Make a new generator function in a new environment
            EnvironmentInfo oldEnvironment = currentEnvironment;
            currentEnvironment = context.getEnvironment(node);
            try {
                BytecodeCompiler oldCompiler = compiler;
    ...
    public BytecodeBundle visitAttribute(Attribute node) throws Exception {
        ...
        switch(node.ctx){
            case AugLoad:
                break; // TODO: when does this happen?
            case AugStore:
                break; // TODO: when does this happen?
            case Del:
    ...
    public BytecodeBundle visitAugAssign(AugAssign node) throws Exception {
        if (node.target instanceof Attribute) {
            ...
            if (subscr.slice instanceof Slice && ((Slice)subscr.slice).step == null) {
                ...
                // TODO: use slicing instructions
            } else {
                // TODO: more code here!
            }
        } else {
            // TODO: SyntaxError: illegal expression for augmented assignment
        }
        lastStatementWasReturn = false;
        return bundle;
    }
    ...
    public BytecodeBundle visitIf(If node) throws Exception {
        ...
        lastStatementWasReturn = false; // TODO: this could be derived from the it tree
        return bundle;
    }
    ...
    public BytecodeBundle visitSubscript(Subscript node) throws Exception {
        ...
        if (node.slice instanceof Slice) {
            ...
            if (slice.step != null) {
                ...
                // TODO: adapt to different contexts
    ...
    public BytecodeBundle visitTryExcept(TryExcept node) throws Exception {
        ...
        for (excepthandlerType handler : node.handlers) {
            if (next != null) {
                ...
                // Pop residue from last comparison
                compiler.visitPop();
            }
        ...
        if (next != null) {
            // No generic target - add a re-raise block
            compiler.visitLabel(next);
            compiler.visitPop();
        ...
        compiler.visitLabel(done);
        lastStatementWasReturn = false; // TODO: this can be derived from the tree
        return bundle;
    }
    ...
    public BytecodeBundle visitUnicode(Unicode node) throws Exception {
        PyUnicode string;
        if (node.s instanceof PyUnicode) {
            string = (PyUnicode)node.s;
        } else if (node.s instanceof PyString) {
            string = ((PyString)node.s).__unicode__();
        } else if (node.s instanceof String) {
            string = new PyUnicode((String)node.s);
        } else {
            string = new PyUnicode(node.s.toString());
        }
        compiler.visitLoadConstant(string);
        return bundle;
    }
    */
    ...
    private BinaryOperator binaryOperator(operatorType op) {
        switch(op){
            ...
            default:
                return null; // TODO: SyntaxError of some sort
        }
    }
