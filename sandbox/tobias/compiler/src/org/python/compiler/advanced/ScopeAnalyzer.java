...
class ScopeAnalyzer extends VisitorBase<Void> {
    static <T> T scan(SyntaxErrorPolicy errorPolicy, PragmaParser pragmas,
            ...
            final ScopeFactory<T> factory, java.util.List<? extends stmt> body,
            boolean moduleCells) throws Exception {
        ScopeMaker<T> base = new ScopeMaker<T>(null, factory, "Module", scopes,
                moduleCells) {
            @Override
            T create(String[] locals, String[] globals, String[] free,
                    String[] cell, String[] purecell) {
                if (purecell.length != 0) {
                    throw new IllegalStateException(
                            "Module can not have free variables.");
                }
                if (globals.length != 0 || free.length != 0) {
                    ...
                    // Free variables correspond to builtins
        ...
        ScopeAnalyzer analyzer = new ScopeAnalyzer(base, errorPolicy, pragmas,
                constants);
    ...
    private static String[] ofStrings(int size) {
        if (size == 0) {
            // Avoid allocation for empty case
    ...
    private ScopeAnalyzer(ScopeMaker<?> scope, SyntaxErrorPolicy errorPolicy,
            PragmaParser pragmas, ConstantPool constants) {
        this.scope = scope;
    ...
    private static abstract class ScopeMaker<T> {
        ...
        ScopeMaker(ScopeMaker<T> parent, ScopeFactory<T> factory, String kind,
                Map<PythonTree, T> scopes, boolean canHaveCell) {
            this.parent = parent;
        ...
        final T complete() throws Exception {
            ...
            return create(assigned.toArray(ofStrings(assigned.size())),
                    global.toArray(ofStrings(global.size())),
                    nonlocal.toArray(ofStrings(nonlocal.size())),
                    cell.toArray(ofStrings(cell.size())),
                    miranda.toArray(ofStrings(miranda.size())));
        }

        abstract T create(String[] locals, String[] globals, String[] free,
                String[] cell, String[] purecell);
        ...
        void param(String name) {
            throw new ParseException(scopeKind
                    + " definitions does not have parameter.");
        }
        ...
        void global(String name) {
            if (nonlocal.contains(name)) {
                throw new ParseException("'" + name
                        + "' is defined as both global and nonlocal.");
            }
            global.add(name);
        }

        /*
         * returns true if it is a cell variable
         */
            ...
            if (canHaveCell) {
                if (assigned.contains(name)) {
                    ...
                    if (parent.require(name)) {
                        // Neither defined or used in this scope
                        miranda.add(name);
                        return true;
                    }
                }
        ...
        void nonlocal(String name) {
            if (parent == null) {
                throw new ParseException("nonlocal definition not allowed on "
                        + scopeKind + " level.");
            }
            ...
            if (!parent.require(name)) {
                throw new ParseException("No definition of nonlocal '" + name
                        + "' in parent scope.");
            }
            nonlocal.add(name);
        }
        ...
        void visitYield() {
            throw new ParseException(scopeKind
                    + " definition may not contain yield.");
        }

        void visitReturn(boolean hasValue) {
            throw new ParseException(scopeKind
                    + " definition may not contain return.");
        }
        ...
        private ScopeMaker<T> classScope(final String name) {
            return new ScopeMaker<T>(this, factory, "Class", scopes, true) {
                ...
                T create(String[] locals, String[] globals, String[] free,
                        String[] cell, String[] purecell) {
                    return factory.createClass(name, locals, globals, free,
                            cell, this.hasStarImport);
                }
            };
        }

        private ScopeMaker<T> functionScope(final String name,
                final boolean is_generator) {
            return new ScopeMaker<T>(this, factory, "Function", scopes, true) {
                ...
                T create(String[] locals, String[] globals, String[] free,
                    ...
                    return factory.createFunction(name,
                            parameters.toArray(ofStrings(parameters.size())),
                            locals, globals, free, cell, this.isGenerator,
                            this.hasStarImport);
                }
                ...
                void nonlocal(String name) {
                    if (parameters.contains(name)) {
                        throw new ParseException("Parameter '" + name
                                + "' may not be declared nonlocal.");
                    }
                    super.nonlocal(name);
                }
                ...
                void visitYield() {
                    if (returnEncountered) {
                        throw new ParseException(
                                "One function may not contain both yield and return (with a value).");
                    }
                    isGenerator = true;
                }
        ...
        final void add(ScopeAnalyzer parent, final ClassDef node) {
            nested.add(new NestedScopeAnalyzer<T>(parent,
                    classScope(node.getInternalName())) {
                @Override
        ...
        final void add(ScopeAnalyzer parent, final FunctionDef node) {
            nested.add(new NestedScopeAnalyzer<T>(parent, functionScope(
                    node.getInternalName(), false)) {
                @Override
                PythonTree traverse() throws Exception {
    ...
    private static abstract class NestedScopeAnalyzer<T> extends ScopeAnalyzer {
    ...
    public Void visitGeneratorExp(GeneratorExp node) throws Exception {
        ...
        comprehension comp = (generators == null || generators.isEmpty()) ? null
                : generators.get(0);
        expr iter = comp != null ? comp.getInternalIter() : null;
        if (iter == null) {
            errorPolicy.astError("No iterator(s) defined for comprehension.",
                    node);
        } else {
            iter.accept(this);
            scope.add(this, node);
