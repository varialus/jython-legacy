 ...
 * @param <E>
 */
final class ScopesBuilder<E> extends VisitorBase<Void> {

    static <T, E> T scan(SyntaxErrorPolicy errorPolicy, PragmaParser pragmas,
            Map<String, ConstantChecker<E>> constNames,
            Map<PythonTree, T> scopes, ScopeFactory<T> factory,
            java.util.List<? extends stmt> body) throws Exception {
        ScopeBuilder<T> base = ScopeBuilder.create(factory, scopes);
        ScopesBuilder<E> builder = new ScopesBuilder<E>(errorPolicy,
                constNames, pragmas);
        builder.push(base);
        try {
    ...
    private final Map<String, ConstantChecker<E>> constNames;
    private final PragmaParser pragmas;

    private ScopesBuilder(SyntaxErrorPolicy errorPolicy,
            Map<String, ConstantChecker<E>> constNames, PragmaParser pragmas) {
    ...
    public Void visitClassDef(ClassDef node) throws Exception {
        ConstantChecker<?> check = constNames.get(node.getInternalName());
        if (check != null) {
            errorPolicy.syntaxError("Cannot define class " + node.getInternalName() + ", "
                    + node.getInternalName() + " is a constant.", node);
        }
        try {
    ...
    public Void visitFunctionDef(FunctionDef node) throws Exception {
        ConstantChecker<?> check = constNames.get(node.getInternalName());
        if (check != null) {
            errorPolicy.syntaxError("Cannot define function " + node.getInternalName()
                    + ", " + node.getInternalName() + " is a constant.", node);
        }
        try {
            current().def(node.getInternalName());
        ...
        function(node, node.getInternalName(), node.getInternalArgs(), node.getInternalBody());
        return null;
    }

    @Override
    public Void visitLambda(Lambda node) throws Exception {
        function(node, "<lambda>", node.getInternalArgs(), Arrays.asList(node.getInternalBody()));
        return null;
    }
    ...
    public Void visitName(Name node) throws Exception {
        ConstantChecker<?> check = constNames.get(node.getInternalId());
        try {
            switch (node.getInternalCtx()) {
            case AugLoad:
            case AugStore:
                throw new ParseException("The name context " + node.getInternalCtx()
                        + " is not supported!");
            case Del:
                if (check != null) {
                    errorPolicy.syntaxError("Cannot delete " + node.getInternalId()
    ...
    public Void visitImport(Import node) throws Exception {
        for (alias al : node.getInternalNames()) {
            ...
            if (constNames.get(name) != null) {
                errorPolicy.syntaxError("Cannot import " + al.getInternalName() + " as "
                        + name + ", " + name + " is a constant.", al);
            } else {
                try {
                    current().def(name);
                } catch (ParseException pe) {
    ...
    public Void visitImportFrom(ImportFrom node) throws Exception {
        ...
        if (node.getInternalNames() == null || node.getInternalNames().isEmpty()
                || (node.getInternalNames().size() == 1 && "*".equals(node.getInternalNames().get(0).getInternalName()))) {
            // import *
    ...
    private void checkAssign(expr target, expr value) {
        if (target instanceof Name) {
            ...
            ConstantChecker<?> check = constNames.get(name.getInternalId());
            if (check != null) {
                if (value == null || !check.acceptValue(value)) {
                    errorPolicy.syntaxError("Cannot assign to " + name.getInternalId()
                            + ", " + name.getInternalId() + " is a constant.", target);
                }
            }
