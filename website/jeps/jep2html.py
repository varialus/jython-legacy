...
-b, --browse
    ...
    browse the local HTML.  If no jep arguments are given, this
    will browse PEP 0.
...
The optional arguments ``jeps`` are either jep numbers or .txt files.
"""
...
PEPURL = 'jep-%04d.html'
PEPCVSURL = ('http://svn.python.org/view/jeps/trunk/jep-%04d.txt')
PEPDIRRUL = 'http://www.python.org/jeps/'


HOST = "dinsdale.python.org"                    # host for update
HDIR = "/data/ftp.python.org/pub/www.python.org/jeps" # target host directory
LOCALVARS = "Local Variables:"

COMMENT = """<!--
This HTML is auto-generated.  DO NOT EDIT THIS FILE!  If you are writing a new
PEP, see http://www.python.org/jeps/jep-0001.html for instructions and links
...
fixpat = re.compile("((https?|ftp):[-_a-zA-Z0-9/.+~:?#$=&,]+)|(jep-\d+(.txt)?)|"
                    "(RFC[- ]?(?P<rfcnum>\d+))|"
                    "(PEP\s+(?P<jepnum>\d+))|"
                    ".")
...
def fixanchor(current, match):
    ...
    elif text.startswith('jep-') and text <> current:
        link = os.path.splitext(text)[0] + ".html"
    elif text.startswith('PEP'):
        jepnum = int(match.group('jepnum'))
        link = PEPURL % jepnum
...
def linkemail(address, jepno):
    parts = address.split('@', 1)
    return ('<a href="mailto:%s&#64;%s?subject=PEP%%20%s">'
            '%s&#32;&#97;t&#32;%s</a>'
            % (parts[0], parts[1], jepno, parts[0], parts[1]))
...
def fixfile(inpath, input_lines, outfile):
    ...
    # convert plaintext jep to minimal XHTML markup
    print >> outfile, DTD
    print >> outfile, '<html>'
    ...
    if basename <> 'jep-0000.txt':
        print >> outfile, '[<b><a href=".">PEP Index</a></b>]'
    if jep:
        try:
            print >> outfile, ('[<b><a href="jep-%04d.txt">PEP Source</a>'
                               '</b>]' % int(jep))
    ...
    for k, v in header:
        if k.lower() in ('author', 'discussions-to'):
            ...
            otherjeps = ''
            for otherjep in re.split(',?\s+', v):
                otherjep = int(otherjep)
                otherjeps += '<a href="jep-%04d.html">%i</a> ' % (otherjep,
                                                                  otherjep)
            v = otherjeps
            ...
            try:
                url = PEPCVSURL % int(jep)
                v = '<a href="%s">%s</a> ' % (url, cgi.escape(date))
            except ValueError, error:
                v = date
        elif k.lower() in ('content-type',):
            url = PEPURL % 9
            jep_type = v or 'text/plain'
            v = '<a href="%s">%s</a> ' % (url, cgi.escape(jep_type))
        else:
            v = cgi.escape(v)
        ...
        if line[0].strip():
            ...
            if basename == 'jep-0000.txt':
                ...
                if len(parts) > 1 and re.match(r'\s*\d{1,4}', parts[1]):
                    ...
                    # This is a jep email address line, so filter it.
                    url = fixemail(parts[-1], jep)
                    if need_pre:
                        print >> outfile, '<pre>'
                        need_pre = 0
                    print >> outfile, re.sub(
...
def get_jep_type(input_lines):
    ...
    for line in input_lines:
        ...
        if not line:
            ...
            jep_type = line.split()[1] or 'text/plain'
            break
        elif line.startswith('jep: '):
...

def find_jep(jep_str):
    """Find the .txt file indicated by a cmd line argument"""
    if os.path.exists(jep_str):
        return jep_str
    num = int(jep_str)
    return "jep-%04d.txt" % num

def make_html(inpath, verbose=0):
    input_lines = get_input_lines(inpath)
    jep_type = get_jep_type(input_lines)
    if jep_type is None:
        print >> sys.stderr, 'Error: Input file %s is not a PEP.' % inpath
        sys.stdout.flush()
        return None
    elif not PEP_TYPE_DISPATCH.has_key(jep_type):
        print >> sys.stderr, ('Error: Unknown PEP type for input file %s: %s'
                              % (inpath, jep_type))
        sys.stdout.flush()
        return None
    elif PEP_TYPE_DISPATCH[jep_type] == None:
        jep_type_error(inpath, jep_type)
        return None
    outpath = os.path.splitext(inpath)[0] + ".html"
    if verbose:
        print inpath, "(%s)" % jep_type, "->", outpath
        sys.stdout.flush()
    outfile = open(outpath, "w")
    PEP_TYPE_DISPATCH[jep_type](inpath, input_lines, outfile)
    outfile.close()
    os.chmod(outfile.name, 0664)
    return outpath

def push_jep(htmlfiles, txtfiles, username, verbose, local=0):
    quiet = ""
    if local:
        if verbose:
            quiet = "-v"
...
def check_requirements():
    ...
    try:
        ...
        PEP_TYPE_MESSAGES['text/plain'] = (
            'Python %s or better required for "%%(jep_type)s" PEP '
    ...
    try:
        ...
        PEP_TYPE_MESSAGES['text/x-rst'] = (
            'Docutils not present for "%(jep_type)s" PEP file %(inpath)s.  '
        ...
        if installed < required:
            PEP_TYPE_DISPATCH['text/x-rst'] = None
            PEP_TYPE_MESSAGES['text/x-rst'] = (
                'Docutils must be reinstalled for "%%(jep_type)s" PEP '
...
def jep_type_error(inpath, jep_type):
    print >> sys.stderr, 'Error: ' + PEP_TYPE_MESSAGES[jep_type] % locals()
    sys.stdout.flush()
...
def main(argv=None):
    ...
    if update:
        push_jep(html, jeptxt, username, verbose, local=local)
        if browse:
            if args:
                for jep in args:
                    browse_remote(jep)
