            ...
            if (targetDirectory.exists()) {
                if (targetDirectory.isDirectory()) {
                    if (targetDirectory.list().length > 0) {
                        throw new ValidationException(Installation.getText(TextKeys.NON_EMPTY_TARGET_DIRECTORY));
                    }
                }
            } else {
                if (targetDirectory.mkdirs()) {
                    throw new ValidationInformationException(Installation
                            .getText(TextKeys.CREATED_DIRECTORY, directory));
                } else {
                    throw new ValidationException(Installation.getText(TextKeys.UNABLE_CREATE_DIRECTORY, directory));
                }
            }
        } else {
            throw new ValidationException(Installation.getText(TextKeys.EMPTY_TARGET_DIRECTORY));
        }
